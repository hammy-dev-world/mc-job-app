//
//  MCFetchTartuEmployeesOperation.swift
//  MCJobApp
//
...
class MCFetchTartuEmployeesOperation: MCOperationBase {
    ...
    override func handleDidFinishedWithResponse(response: Data!) {
        do {
            ...
            for i in 0..<employeesArray.count { // Appending new key/value in the dictionary of each employee
                ...
                employee[String(Substring(MCEmployeeRootObject.EmployeeCodingKeys.location.rawValue))] = EmployeeLocation.kTartuLocationName
            ...
            do {
                ...
                let tartuCachable = TartuEmployeesListCachable(value: responseData)
                
                cacher.persist(item: tartuCachable) { url, error in
                    if let error = error {
    ...
    private func startFetchingEmployeesOperation() {
        let router = Router<TartuNetworkManager>()

        router.request(.getTartuEmployees()) { data, response, error in
